...
export interface NpcAnalysis {
  ...
  relationship?: string;
  /** FATE backstop (shared/npcFate.ts): set ONLY when the scene concretely showed
   *  a KNOWN character dying ("dead") or leaving permanently ("gone") that no
   *  combat recorded — executed in narration, spaced, shipped off. Never a guess;
   *  requires `id` (a new figure can't have a fate worth recording). */
  fate?: "dead" | "gone";
  /** FACTION ALLEGIANCE (HANDOFF_NPC_CANON Task B): set ONLY when the scene made
   *  a KNOWN character's faction allegiance concrete — must be one of the KNOWN
   *  FACTION IDS fed in the prompt. Applied set-once (allegiance CHANGES are a
   *  separate, unbuilt slice) so a generated fixer's faction stops drifting
   *  scene to scene. */
  factionId?: string;
}
...
export interface FactAnalysis {
  text: string;
  entityRefs?: string[];
  /** Only the analyst may set this — a term whose loss would contradict the
   *  story (a deal's exact split, a debt, a kinship). The live turn path
   *  can't pin (TurnPlan has no such field). */
  pinned?: boolean;
}
...
export async function analyzeScene(
  ...
  opts: {
    ...
    /** Canon factions the analyst may pin an NPC's allegiance to (Task B) —
     *  the parse only trusts a factionId that appears in this list. */
  ...
  const factsList = opts.establishedFacts?.length
    ...
    : "(none)";
  const factionRoster = opts.factions?.length
    ? opts.factions.map((f) => `${f.id} = ${f.name}`).join(", ")
    : "(none)";
  const user =
    `KNOWN NPCs (id = name: current description):\n${roster}\n\n` +
    `OPEN THREADS (id = title):\n${threadRoster}\n\n` +
    `ESTABLISHED FACTS (already recorded — do NOT repeat):\n${factsList}\n\n` +
    `KNOWN FACTION IDS (use one of these EXACTLY for a "factionId", or omit): ${factionRoster}\n\n` +
  ...
  const knownFactionIds = new Set((opts.factions ?? []).map((f) => f.id));
    ...
    const npcUpdates: NpcAnalysis[] = (Array.isArray(parsed.npcs) ? parsed.npcs : [])
      ...
      .map((u: Record<string, unknown>): NpcAnalysis => {
        ...
        return {
          ...
          // Fate only rides a trusted KNOWN id — a new figure can't die into canon.
          fate:
            (u.fate === "dead" || u.fate === "gone") && id && knownIds.has(id)
              ? (u.fate as "dead" | "gone")
              : undefined,
          // Faction only rides a REAL known faction id — an invented one is dropped.
          factionId: u.factionId && knownFactionIds.has(String(u.factionId)) ? String(u.factionId) : undefined,
        };
      })
