...
export async function runJsonTurn(input: JsonTurnInput): Promise<JsonTurnResult> {
  ...
  // The reroute can RESOLVE the whole fight inside this turn (an opening-shot
  // crit kill, or the player dropped in the counter) — when it does, remember the
  // authoritative lines: the model narrates from context alone here and the live
  // failure mode is INVERSION (Piotr's crit killed Doran; DeepSeek narrated Piotr
  // shot down + "killed in action"). combat_resolved_realign below fixes it.
  let fightResolvedLines: string | null = null;
  const openFight = (skill: string, dc: number): CombatState | null => {
    ...
    emit(r.lines);
    if (!r.combat && r.lines.length) fightResolvedLines = r.lines.join(" · ");
    return r.combat;
  };
  ...
  // DEATH-TITLE SCRUB: only the ENGINE kills a character. A model sceneEnd titled
  // with the player's death while they're alive and standing would immortalize a
  // false death in scene memory (the live "Piotr Calloway — killed in action"
  // title on the turn Piotr actually WON the fight). Neutralize before the scene
  // closes with it.
  {
    const livePc = runtime.state.characters.find((c) => c.kind === "pc");
    const pcStanding = !!livePc && !(livePc.injuries ?? []).some((i) => i.name === "Dead" || i.name === "Downed");
    if (pcStanding && plan.sceneEnd?.title && /\b(dead|death|dies|died|killed|slain|kia)\b/i.test(plan.sceneEnd.title)) {
      toolCalls.push("scrub_death_title");
      plan = { ...plan, sceneEnd: { ...plan.sceneEnd, title: "The dust settles" } };
    }
  }
  ...
  // ── The reroute RESOLVED the whole fight inside this turn (opening-shot kill,
  //    or the player dropped in the counter). No active fight means the
  //    combat_open_realign above never fires — and the model, narrating from the
  //    ⚔ lines alone, INVERTED the result live (Piotr's crit killed Doran;
  //    DeepSeek wrote Piotr shot down, "killed in action", no choices). Realign
  //    against the authoritative lines, and take the realign's choices when the
  //    plan offered none (the false sceneEnd emptied them). ──
  let resolvedFightChoices: TurnPlan["choices"] | null = null;
  if (fightResolvedLines && !combat?.active) {
    const livePc = runtime.state.characters.find((c) => c.kind === "pc");
    const pcStanding = !!livePc && !(livePc.injuries ?? []).some((i) => i.name === "Dead" || i.name === "Downed");
    toolCalls.push("combat_resolved_realign");
    messages.push({ role: "assistant", content: JSON.stringify({ narration }) });
    messages.push({
      role: "user",
      content:
        `That exchange was ALREADY RESOLVED by the engine — the authoritative result:\n${fightResolvedLines}\n` +
        `Honor it EXACTLY. The ☠ line names who FELL${pcStanding ? " — an ENEMY. The PLAYER WON: they are alive, standing, and unhurt except where a line shows damage to THEM. NEVER narrate the player dead, dying, or downed — only the engine decides that" : ""}. ` +
        `Re-narrate the exchange to match (2-4 vivid sentences, present tense, no dice math) and offer 2-4 "choices" for what happens next in the aftermath. REPLACE your previous narration.`,
    });
    const outcome = await plannedCall(false);
    if (outcome.narration.trim()) narration = outcome.narration;
    if (outcome.choices.length) resolvedFightChoices = resolveChoiceChecks(outcome.choices, pc);
  }
  ...
  let choices = plan.sceneEnd || combat ? [] : plan.choices.slice(0, cap);
  // A resolved-fight realign supplies the aftermath choices — they win whenever
  // the plan left none (the false "killed in action" sceneEnd emptied them), so
  // the beat after a decisive fight never degrades to generic fallback chips.
  if (!combat && choices.length === 0 && resolvedFightChoices?.length) {
    choices = resolvedFightChoices.slice(0, cap);
  }
