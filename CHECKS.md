...
| **NPC fate — engine-recorded death** | `shared/npcFate.ts` (`applyCombatDeaths`/`markNpcFate`), wired in `resolveCombatRound` (the single dispatcher every fight path crosses); analyst `fate` backstop (`summarizer` → `applyAnalystUpdates`) for narrated deaths no fight recorded | fight end + scene analysis | `Npc.status` existed and retrieval FILTERED gone NPCs, but nothing ever WROTE it — a named cast NPC killed in combat stayed "alive" forever, free to be re-narrated later (the cast-level "dead guard comes back" class). Now a defeated enemy matching a living cast NPC (exact base name, never PCs/crew, never generic mooks) is recorded dead with a relation-log note the People panel shows; the analyst records deaths/departures the scene showed. THE DEAD STAY DEAD: gone NPCs can't be re-marked present by inference (`jsonTurn` gate) or by the model's explicit `npcs[]` (`applyPlan` gate) |
